"""
ONE SCRIPT TO RULE THEM ALL (30k–50k scale) — ENHANCED VERSION 2
...
UMAP_FP_PARAMS = dict(
    n_neighbors=15,   # configuration optimized for better local structure. JOC 20260112 
    min_dist=0.05,   
    n_components=2,
...
DATA_CONDITION = "B"  # Options: "A" (Literature+Hits), "B" (Library+Hits), "ALL" (all data)
...
def main():
    ...
    if DATA_CONDITION == "A":
        ...
        condition_label = "Lit_Hits"
        print(f"\nDATA CONDITION A: Keeping Literature + Hits only")
    elif DATA_CONDITION == "B":
        df = df[df[SOURCE_COL].isin(["library", "hit"])].copy()
        condition_label = "Lib_Hits"
        print(f"\nDATA CONDITION B: Keeping Library + Hits only")
    else:
        condition_label = "All"
        print(f"\nDATA CONDITION: Keeping all data (Literature + Library + Hits)")
    ...
    out_csv_fp = os.path.join(OUTPUT_DIR, f"structural_umap_mapchiral_{condition_label}.csv")
    df_fp.to_csv(out_csv_fp, index=False)
    print(f"Saved structural UMAP CSV: {out_csv_fp}")

    out_svg_fp = os.path.join(FIG_DIR, f"structural_umap_mapchiral_{condition_label}_hits_highlights.svg")
    plot_umap_enhanced(
    ...
    # STEP 5B — FPM-normalized UMAP (complexity density analysis)
    if ENABLE_FPM_NORMALIZATION:
        ...
        reducer_fpm = umap.UMAP(metric=minhash_distance, **UMAP_FP_PARAMS)
        ...
        out_csv_fpm = os.path.join(OUTPUT_DIR, f"structural_umap_fpm_normalized_{condition_label}.csv")
        df_fp.to_csv(out_csv_fpm, index=False)
        print(f"Saved FPM-normalized UMAP CSV: {out_csv_fpm}")
        
        out_svg_fpm = os.path.join(FIG_DIR, f"structural_umap_fpm_normalized_{condition_label}_hits_highlights.svg")
        plot_umap_enhanced(
            df_fp, "UMAP_FPM_1", "UMAP_FPM_2",
            "Complexity Density Space (FPM-normalized MAPchiral + UMAP) with Hits and Highlights",
            out_svg_fpm, hit_color_map
        )
    ...
    out_csv_desc = os.path.join(OUTPUT_DIR, f"descriptor_umap_6desc_{condition_label}.csv")
    df_desc.to_csv(out_csv_desc, index=False)
    print(f"Saved descriptor UMAP CSV: {out_csv_desc}")

    out_svg_desc = os.path.join(FIG_DIR, f"descriptor_umap_6desc_{condition_label}_hits_highlights.svg")
    plot_umap_enhanced(
